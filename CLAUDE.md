...
Breaking changes: add `!` after type (e.g., `feat(envelope)!: change Envelope variants`)

See `AGENTS.md` > Conventional Commits for the full specification.

## Build & Test

Use **`mise run`** for all operations -- not raw `moon` commands:
...
- Release workflow: validate -> tag -> publish
